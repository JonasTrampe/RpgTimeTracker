...
public sealed partial class SceneLibraryItemViewModel : ObservableObject, ITaggable
{
    ...
    /// <summary>
    ///     Whether this is MainWindowViewModel.ActiveScene right now - purely local UI state,
    ...
    ///     which one it is without every consumer having to compare against ActiveScene itself.
    /// </summary>
    [ObservableProperty] private bool _isActive;

    /// <summary>
    ///     See NpcGmInfoBlockViewModel.IsPreviewMode's doc comment - same purely-local,
    ///     not-persisted purpose here.
    /// </summary>
    ...
    /// <summary>
    ///     Purely local UI state (not persisted) - the ComboBox selection in each "add" row
    ...
    ///     "currently selected item" concept.
    /// </summary>
    ...
    /// <summary>
    ///     Editable text form of StartDate, following CalendarEntryViewModel.
    ...
    ///     timebound, see this class's doc comment.
    /// </summary>
    [ObservableProperty] private string _startDateText;

    /// <summary>
    ///     See NpcLibraryItemViewModel._suppressChangeNotifications' doc comment - same
    ///     purpose here, set while MainWindowViewModel.FromSceneLibraryEntryDto is reconstructing
    ///     this Scene from its saved DTO.
    /// </summary>
    ...
    /// <summary>
    ///     A Scene can bundle several Images/Maps at once - see this class's doc comment for
    ///     why activation doesn't auto-send them.
    /// </summary>
    ...
    /// <summary>
    ///     Phase 3's scene-scoped timeline - see this class's doc comment for the
    ...
    ///     reconstructing this Scene from its saved DTO.
    /// </summary>
    ...
    /// <summary>
    ///     Same Timers/Alarms/IntervalEvents, each wrapped in a TimelineDisplayItemViewModel
    ...
    /// <summary>
    ///     Freeform Tag Ids attached to this Scene - separate from Scene membership on other
    ///     library items, a different, explicit mechanism (see Tag's doc comment).
    /// </summary>
    ...
    partial void OnIsDescriptionPreviewModeChanged(bool value)
    {
        OnPropertyChanged(nameof(DescriptionPreviewToggleIcon));
    }

    [RelayCommand]
    private void ToggleDescriptionPreview()
    {
        IsDescriptionPreviewMode = !IsDescriptionPreviewMode;
    }
    ...
    [RelayCommand]
    private void RemoveSound(SoundLibraryItemViewModel sound)
    {
        Sounds.Remove(sound);
    }
    ...
    [RelayCommand]
    private void RemoveImage(MediaLibraryItemViewModel image)
    {
        Images.Remove(image);
    }
    ...
    [RelayCommand]
    private void RemovePlaylistFromBundle(PlaylistViewModel playlist)
    {
    ...
    /// <summary>
    ///     Creates a new Timer directly owned by this Scene, with the same sensible
    ///     defaults AddTimer's global equivalent falls back to - the GM edits Name/Duration
    ///     afterwards via the item's own existing ApplyEdits UI, same as the global timeline.
    /// </summary>
    [RelayCommand]
    ...
    /// <summary>
    ///     Public (unlike RemoveSound et al.) so MainWindowViewModel.FromSceneLibraryEntryDto
    ///     can pass it directly as TimerItemViewModel.FromDto's onDeleteRequested callback while
    ///     reconstructing this Scene's saved timeline.
    /// </summary>
    public void RemoveTimer(TimerItemViewModel vm)
    {
        Timers.Remove(vm);
    }
    ...
    public void RemoveAlarm(AlarmItemViewModel vm)
    {
        Alarms.Remove(vm);
    }
    ...
    /// <summary>See RemoveTimer's doc comment - same reasoning.</summary>
    public void RemoveIntervalEvent(IntervalEventItemViewModel vm)
    {
