...
public readonly struct GameInstant : IEquatable<GameInstant>, IComparable<GameInstant>
{
    ...
    public GameInstant Add(TimeSpan delta) => new(TotalSeconds + (long)delta.TotalSeconds);

    public TimeSpan Subtract(GameInstant other) => TimeSpan.FromSeconds(TotalSeconds - other.TotalSeconds);

    public int CompareTo(GameInstant other) => TotalSeconds.CompareTo(other.TotalSeconds);

    public bool Equals(GameInstant other) => TotalSeconds == other.TotalSeconds;
    ...
    public static GameInstant operator +(GameInstant instant, TimeSpan delta) => instant.Add(delta);

    public static TimeSpan operator -(GameInstant a, GameInstant b) => a.Subtract(b);

    public static bool operator ==(GameInstant a, GameInstant b) => a.Equals(b);

    public static bool operator !=(GameInstant a, GameInstant b) => !a.Equals(b);

    public static bool operator <(GameInstant a, GameInstant b) => a.TotalSeconds < b.TotalSeconds;
