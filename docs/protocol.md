...
| `map.tokenUpsert` | `MapTokenSnapshotDto` (id, floorId, x, y, name?, detail?, iconGlyph?, isCurrentTurn) | SL adds/updates one map token - the whole resolved snapshot, always, never a partial diff (a token's payload is tiny, unlike fog cells - a drag, a floor reassignment, a relink, or a visibility-toggle change are all just another `map.tokenUpsert`). Name/Detail/IconGlyph are resolved server-side from the linked Character+Variant or Point of Interest (or the token's own freeform fields) and are `null` when that field's player-visible toggle is off - the client applies no visibility logic of its own. Also covers a token becoming newly visible (e.g. `HiddenUntilRevealed` and its cell was just painted revealed) |
